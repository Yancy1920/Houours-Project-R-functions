add_noise_to_smooth <- function(smooth){
  ...
  for (iPoint in 1:dim(smooth_profiles)[3]) {
    smooth_profiles[, 1, iPoint] <-
      smooth_profiles[, 1, iPoint] + x1_114[iPoint + 100]
  }
  # dye 2
  for (iPoint in 1:dim(smooth_profiles)[3]) {
    smooth_profiles[, 2, iPoint] <-
      smooth_profiles[, 2, iPoint] + x2_113[iPoint + 100]
  }
  # dye 3
  for (iPoint in 1:dim(smooth_profiles)[3]) {
    smooth_profiles[, 3, iPoint] <-
      smooth_profiles[, 3, iPoint] + x3_112[iPoint + 100]
  }
  # dye 4
  for (iPoint in 1:dim(smooth_profiles)[3]) {
    smooth_profiles[, 4, iPoint] <-
      smooth_profiles[, 4, iPoint] + x4_212[iPoint + 100]
  }
  # dye 5
  for (iPoint in 1:dim(smooth_profiles)[3]) {
    smooth_profiles[, 5, iPoint] <-
      smooth_profiles[, 5, iPoint] + x5_304[iPoint + 100]
  }
  # dye 6
  for (iPoint in 1:dim(smooth_profiles)[3]) {
    smooth_profiles[, 6, iPoint] <-
      smooth_profiles[, 6, iPoint] + x6_113[iPoint + 100]
  }
