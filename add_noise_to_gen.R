add_noise_to_gen <- function(gen){
  # Add simulated noise onto gen profiles
  ...
  gen_profiles <- gen$gen_profiles
  gen_paths <- gen$gen_paths
  # add noise
  
  # dye 1
  for(iPoint in 1:dim(gen_profiles)[3]){
    gen_profiles[,1,iPoint] <- gen_profiles[,1,iPoint] + x1_114[iPoint+100]
  }
  # dye 2
  for(iPoint in 1:dim(gen_profiles)[3]){
    gen_profiles[,2,iPoint] <- gen_profiles[,2,iPoint] + x2_113[iPoint+100]
  }
  # dye 3
  for(iPoint in 1:dim(gen_profiles)[3]){
    gen_profiles[,3,iPoint] <- gen_profiles[,3,iPoint] + x3_112[iPoint+100]
  }
  # dye 4
  for(iPoint in 1:dim(gen_profiles)[3]){
    gen_profiles[,4,iPoint] <- gen_profiles[,4,iPoint] + x4_212[iPoint+100]
  }
  # dye 5
  for(iPoint in 1:dim(gen_profiles)[3]){
    gen_profiles[,5,iPoint] <- gen_profiles[,5,iPoint] + x5_304[iPoint+100]
  }
  # dye 6
  for(iPoint in 1:dim(gen_profiles)[3]){
    gen_profiles[,6,iPoint] <- gen_profiles[,6,iPoint] + x6_113[iPoint+100]
  }
  return(gen_profiles)
