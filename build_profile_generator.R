...
build_profile_generator <- function(settings){
  ...
  layer1_input_shape <- c(number_of_dyes, number_scanpoint_in_input_layer, number_channels) # 6,5000,1
  
  #build the ANN
  profile_inputs <- layer_input(shape=layer1_input_shape, name="profile_input_layer")
  
  profile_downsize <- profile_inputs %>%# 6 -> 16
    layer_conv_2d(filters = 16, strides=c(1, 10), kernel_size = c(1, 10), activation = "relu", padding = 'same', name = "downsize1")
  
  profile_horizontal_model <- profile_downsize %>%  # 16->32->32
  ...
  profile_horizontal_model_downsize2 <- profile_horizontal_model %>%  # 32->64->64
  ...
  profile_horizontal_model_downsize_centre <- profile_horizontal_model_downsize2 %>%  # 64->128
  ...
  profile_vertical_model <- profile_downsize %>%  # 256 -> 128 -> 32
  ...
  #hold numbers 0 to 1
  number_input_layer <- layer_input(shape=c(6,500, 1), name="numbered_input_layer")
  
  # 6 profile vertical models are connected in axis1
...
#  conatenate_500_layers <- layer_concatenate(inputs = list(profile_horizontal_upsize1, duplicate_vertical_layer), axis = 3,  name = "final_concat")
  ...
  output_layer <- layer_concatenate(inputs = list(profile_inputs, final_upsample), axis = 3) %>%
    ...
    layer_batch_normalization()%>%
    layer_spatial_dropout_2d(rate = 0.25) %>%
    layer_conv_2d(filters = 1, strides=c(1, 1), kernel_size = c(1, 5), padding = 'same', name = "profile_output_L3") %>%
...
#  complete_model <- keras_model(inputs = profile_inputs, outputs = output_layer)
  ...
  complete_model %>% compile(
    optimizer_adam(learning_rate = 2e-4, beta_1 = 0.5),
