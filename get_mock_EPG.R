get_mock_EPG <- function(all_EPG_data, plot_EPG = FALSE, EPG_plot_savename = "EPG.jpg", context_reach = 10){
  ...
  #sets up x values
  startScan <- 4000 - context_reach
  endScan <- 9000 + context_reach - 1
  ...
  for (dye in 1:number_of_dyes){
    ...
    for (locus in 1:number_loci_in_dyeset){
      ...
      peaks_at_profile_and_locus <- as.numeric(all_EPG_data[all_EPG_data$Locus==current_locus,]$Allele)
