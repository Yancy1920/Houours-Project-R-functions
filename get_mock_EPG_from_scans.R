get_mock_EPG_from_scans <- function(profile_data){
 ...
 # profile_data <- raw_profile
  ...
  all_GF_Dyes <- c("blue",    "blue", "blue",    "blue",   "blue", "dark green", "dark green", "dark green", "dark green", "dark green", "dark green", "orange", "orange",  "orange", "orange", "red",      "red",    "red",     "red",    "red",  "brown", "purple",   "purple",  "purple",  "purple")
  ...
  for (dye in 1:number_of_dyes){
    ...
    mockEPGPlot[[dye]] <- ggplot(data = profile_data[startScan:endScan,]) +
                          geom_line(aes_string(x = "scan", y = paste("dye", dye, sep="")), col = current_dye, linewidth = 0.1) +
